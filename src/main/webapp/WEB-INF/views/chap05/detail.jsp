...
<body>
   ...
   <script>
      ...
      const bno = '${b.boardNo}'; // 게시물 번호를 전역변수화.
      ...
      function fetchGetReplies(pageNum = 1) { // 댓글 목록 요청시 페이지번호 전달(전달 안되면 기본값 1)
         ...
         fetch(`\${URL}/\${bno}/page/\${pageNum}`)
            ...
            .then(replyList => {
               ...
               // 기존에는 model에 담아서 jsp로 전달했고, jsp쪽에서 el을 이용해서 화면에 뿌렸조.
      ...
      if ($addBtn) {
         $addBtn.onclick = e => {
            ...
            // 공백이 제거된 값을 얻음.
            const textValue = $replyText.value.trim();
            const writerValue = $replyWriter.value.trim();
            ...
            if (textValue === '') {
               ...
               return;
            } else if (writerValue.length < 2 || writerValue.length > 8) {
               alert('댓글 작성자는 2글자에서 8글자 사이로 작성하세요!');
               return;
            }

            // 서버로 보낼 데이터 준비. (JavaScript 형태)
            const payload = {
               text: textValue,
               author: writerValue,
               bno: bno
            ...
            fetch(URL, requestInfo)
               ...
               // 콜백함수의 매개변수로 응답정보가 담긴 Response 객채가 전달되고,
               // Response 객채에서 json 데이터를 꺼내고 싶으면 json(), 단순 텍스트라면 text().
               .then(res => {
                  console.log(res.status); // 서버에서 전달한 응답 상태코드
                  if (res.status === 200) {
