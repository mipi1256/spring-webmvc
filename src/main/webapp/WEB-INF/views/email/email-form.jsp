...
<body>
    ...
    <script>
        ...
        document.getElementById('mail-check-btn').onclick = () => {
            ...
            // 서버 이메일 전송 해주는 코드 -> Connect with Controller
            fetch('/members/email', {
                    method: 'post',
                    headers: {
        ...
        document.getElementById('mail-check-input').onblur = e => {
            ...
            if (inputCode === code) {
                document.getElementById('mailCheckMsg').textContent = '인증번호 일치합니다!';
                document.getElementById('mailCheckMsg').style.color = 'skyblue';
                e.target.style.display = none;
            } else {
                document.getElementById('mailCheckMsg').textContent = '인증번호 다시 확인해주세요!';
