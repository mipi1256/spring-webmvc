...
public class MailSenderService {
   ...
   public String joinEmail(String email) { // 컨트롤러로부터 이멜을 전달받음.

      int authNum = makeRandomNumber();
      String setFrom = "mipi1257@naver.com"; // EmailConfig에 설정한 발신용 이메일과 똑같이!
      ...
      String content = "홈페이지 가입을 신청해 주셔서 감사합니다." +
            ...
            "해당 인증 번호를 인증번호 확인란에 가입해 주세요."; // 이메일 삽입할 내용 (더 꾸며보세요).

      mailSend(setFrom, toMail, title, content);

      return Integer.toString(authNum); // 인증 번호 보내기
   }
   ...
   private void mailSend(String setFrom, String toMail, String title, String content) {
      ...
      try {
         ...
         /*
            ...
            생성자의 매개값으로 MimeMessage 객체, boolean type, 문자 인코딩 설정
