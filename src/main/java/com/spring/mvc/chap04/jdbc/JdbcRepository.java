...
public class JdbcRepository {
   ...
   // 데이터베이스 커넥션 언기 (Connection 객체 얻기 -> 데이터베이스 접속을 담당하는 객체
   ...
   public void delete(int id) {
      ...
      try {
         ...
         //String sql = "DELETE FROM person WHERE id = ?"; 값이 하나면 아래 하면 된다.
         String sql = "DELETE FROM person WHERE id = " + id;

         PreparedStatement pstmt = conn.prepareStatement(sql);
         //pstmt.setInt(1, id); 값이 하나만 선택해서 ? 필요 없다.
   ...
   public List<Person> findAll() {
      ...
      try {
         ...
         while (rs.next()) {
            // 특정 한 행이 지목된 상태 -> 행의 컬럼을 지목해서 데이터를 가져옵니다. (타입 확인 필요합니다)
            int id = rs.getInt("id");
