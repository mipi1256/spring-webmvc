...
public class ScoreController {
   ...
   // 의존객체는 붙변성을 띠게 작성하는 것이 좋다. -> RequiredArgsConstructor로 초기화 진행.
   private final ScoreService service;

   /*
   FM 방식으로 -> 롬북으로 대체 (RequiredArgsConstructor) 생성자가 단 하나라면 autowired를 생략할 수 있음.
   @Autowired
   ...
   */
   ...
   public String register(ScoreRequestDTO dto) {
      ...
      /*
         ...
         - foward할 때는 포워딩할 파일의 경로를 적습니다 (/views/chap04/score-list.jsp)

         - 리다이렉트는 요청후에 자동응답이 나가고
         2번째 자동요청이 들어오면서 2번째 응답을 내보냄
         - 따라서 2번째 요청의 URL로 자동 변경됨
         - redirect 할 때 다시 들어왔으면 하는 요청 url을 적는 것 (/score/list -> 목록 요청)
      */
   ...
   public String detail(int stuNum, Model model) {
      ...
      // 상제보기 이기 때문에 DTO가 아닌 Entity를 담아서 jsp로 보냅니다.
